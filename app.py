...
# ══════════════════════════════════════════════════════════════
#  API KEY — Streamlit Secrets'tan otomatik alınır
#  Kullanıcının UI'da bir şey girmesine gerek yok
# ══════════════════════════════════════════════════════════════
try:
...
if not API_KEY:
    st.error(
        "🔑 **API Key bulunamadı.**\n\n"
        "Lütfen `.streamlit/secrets.toml` dosyasına veya "
        "Streamlit Cloud Secrets kısmına şunu ekle:\n\n"
        "```toml\nGOOGLE_API_KEY = \"AIza...\"\n```"
    )
    st.stop()
...
A: IE 300 is for 3rd-year students focusing on basic engineering applications. IE 400 is for 4th-year students and focuses on advanced engineering problems such as system design and process improvement. IE 300 must be completed before IE 400.
 
Q: Do I need to have IE 200 completed before IE 300?
A: Yes. Successful completion of IE 200 is a prerequisite for IE 300.
 
Q: Can my internship report be in Turkish?
A: Yes. The internship report can be written in either English or Turkish.
 
Q: What happens if my internship is rejected?
A: You will be asked to provide additional documents or information. If corrections are made, it will be re-evaluated. In serious cases you may need to redo the internship.
 
Q: How do I start the application?
A: Go to sp-ie.metu.edu.tr, fill in the online application form with company details and internship dates, upload required documents, and wait for department approval.
"""
...
SCOPE_KEYWORDS = [
    ...
    "prerequisite", "reject", "abroad", "report", "turkish", "english",
]
...
# ══════════════════════════════════════════════════════════════
#  FAISS VECTOR STORE  (runs once, cached for the session)
# ══════════════════════════════════════════════════════════════
...
"For the most accurate and up-to-date information, please visit sp-ie.metu.edu.tr or contact the IE Department secretary."
Use bullet points when listing items. Be concise, clear, and professional.
...
with st.sidebar:
    ...
    st.caption("Click a question to ask it instantly:")
    ...
    with st.chat_message("assistant"):
        if is_out_of_scope(user_input):
            answer = (
                ...
                "such as the application process, required documents, logbook, report writing, "
            ...
            with st.spinner("Thinking…"):
                try:
                    ...
                    answer = (
                        ...
                        "Please make sure the API key in your secrets file is correct."
                    )
                    st.markdown(answer)
